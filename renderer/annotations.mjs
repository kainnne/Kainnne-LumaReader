...
export function createAnnotations({options,getText,sourceRanges}){
 ...
 const actionGroup=document.createElement('div');actionGroup.className='luma-selection-items';bar.append(actionGroup);
 const dock=document.createElement('button');dock.type='button';dock.className='compact-button luma-selection-dock';
 let placement='center';try{placement=localStorage.getItem('lumareader-annotation-dock')==='right'?'right':'center';}catch{}
 function updateDock(){bar.dataset.placement=placement;const label=placement==='right'?tr('移回下方中央','Move to bottom center'):tr('移到右下角','Move to bottom right');dock.title=label;dock.setAttribute('aria-label',label);dock.innerHTML=`<svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="${placement==='right'?'M12 5l-5 5 5 5':'M8 5l5 5-5 5'}"/></svg>`;}
 dock.onpointerdown=e=>e.preventDefault();dock.onclick=()=>{placement=placement==='right'?'center':'right';try{localStorage.setItem('lumareader-annotation-dock',placement);}catch{}updateDock();};updateDock();
 const retry=document.createElement('button');retry.type='button';retry.className='compact-button luma-annotation-retry';retry.hidden=true;retry.textContent=tr('重試儲存','Retry save');retry.onpointerdown=e=>e.preventDefault();
 ...
 for(const a of actions){const b=document.createElement('button');b.type='button';b.className='compact-button';b.dataset.annotationAction=a.id;b.textContent=({image:'▧',highlight:'▰',comment:'▤'})[a.icon]+' '+a.label;b.onpointerdown=e=>e.preventDefault();b.onclick=()=>action(a);actionGroup.append(b);}
 bar.append(retry,dock);
 ...
 function update(){if(!view)return;const selection=getSelection();const encoded=JSON.stringify(selection);if(encoded!==lastEvent){lastEvent=encoded;options.onSelection?.(selection);}actionGroup.hidden=!options.writable||!selection.canAnnotate||!actions.length;bar.hidden=actionGroup.hidden&&retry.hidden;const changed=items().filter(i=>i.status!=='active').length;listButton.textContent=tr('待確認的註記','Annotations to review');listButton.hidden=!changed;}

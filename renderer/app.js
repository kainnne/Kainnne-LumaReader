(function () {
  ...
  async function saveEditing(){
    ...
    try{
      const result=await window.lumaDesktop.saveDocument({path:state.currentPath,text:sourceEditorEl.value,expectedModifiedNs:state.modifiedNs,expectedRevision:state.documentRevision});
      if(!result?.ok){showToast(result?.code==="DOCUMENT_CHANGED"?t("changedExternally"):(result?.message||t("saveFailed")));return;}
      state.rawText=result.document.text||"";state.renderText=result.document.renderText||result.document.text||"";state.modifiedNs=result.document.modifiedNs??state.modifiedNs;state.documentRevision=result.document.revision??null;state.currentBase=result.document.base||state.currentBase;
  ...
  function renderTree(){const query=searchEl.value.trim().toLocaleLowerCase(state.language);const enabled=state.files.filter(currentFormatIsEnabled);const visible=query?enabled.filter((file)=>file.path.toLocaleLowerCase(state.language).includes(query)):enabled;treeEl.innerHTML="";if(!visible.length){const p=document.createElement("p");p.className="sidebar-empty";p.textContent=state.libraryRoot?t("noMatches"):t("noLibrary");treeEl.appendChild(p);return;}if(state.sourceType==="project"&&state.currentPath){const parts=state.currentPath.split("/");parts.pop();for(let i=1;i<=parts.length;i++)state.openFolders.add(parts.slice(0,i).join("/"));}const autoOpenFolders=query?new Set(window.LumaReaderUtils.ancestorFolderPaths(visible)):null;renderTreeNode(treeFromFiles(visible),treeEl,"",autoOpenFolders);}

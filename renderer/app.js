(function () {
  ...
  const state = {
    ...
    creatingDocument:false,createDirectory:""
  };
  ...
  function updateCreateAvailability(){newMarkdownButtonEl.disabled=!state.libraryRoot||state.creatingDocument||!window.lumaDesktop?.createDocument;}
  function updateLibraryDisplay(){libraryRootEl.textContent=state.libraryRoot?folderName(state.libraryRoot):t("noLibraryShort");libraryRootEl.title=state.libraryRoot||t("noLibraryShort");updateCreateAvailability();}
  ...
  function currentDirectoryLabel(directory=state.createDirectory){const root=folderName(state.libraryRoot);return directory?`${root} / ${directory}`:root;}
  ...
  function updateNewMarkdownDialog(){
    ...
    const previewName=markdownPreviewName(),destination=currentDirectoryLabel();newMarkdownSuffixEl.hidden=newMarkdownNameEl.value.trim().toLowerCase().endsWith(".md");newMarkdownDestinationEl.textContent=destination;newMarkdownDestinationEl.title=state.createDirectory?`${state.libraryRoot}/${state.createDirectory}`:(state.libraryRoot||"");newMarkdownConfirmationEl.textContent=previewName?formatTemplate("createMarkdownQuestion",{name:previewName,folder:destination}):"";newMarkdownCreateEl.disabled=state.creatingDocument||!previewName;updateCreateAvailability();
  }
  function showNewMarkdownError(message){newMarkdownErrorEl.textContent=message;newMarkdownErrorEl.hidden=!message;}
  function openNewMarkdownDialog(){
    if(blockWhileEditing())return;if(!state.libraryRoot){showToast(t("libraryRequired"));return;}state.createDirectory=currentDirectoryPath();newMarkdownNameEl.value="";showNewMarkdownError("");updateNewMarkdownDialog();if(!newMarkdownDialogEl.open)newMarkdownDialogEl.showModal();requestAnimationFrame(()=>newMarkdownNameEl.focus());
  }
  function closeNewMarkdownDialog(){if(state.creatingDocument)return;if(newMarkdownDialogEl.open)newMarkdownDialogEl.close();newMarkdownButtonEl.focus();}
  ...
  async function createMarkdownDocument(event){
    ...
    try{
      const result=await window.lumaDesktop.createDocument({directory:state.createDirectory,name});
      ...
      cancelDocumentRequest();revealCreatedDocument(result.document);if(newMarkdownDialogEl.open)newMarkdownDialogEl.close();await consumePayload(result.document,{requestId:state.documentRequestId});beginEditing();showToast(t("documentCreated"));
  ...
  document.addEventListener("keydown",(event)=>{const target=event.target,key=event.key.toLowerCase(),command=event.metaKey||event.ctrlKey;if(newMarkdownDialogEl.open&&event.key==="Escape"){event.preventDefault();closeNewMarkdownDialog();return;}if(command&&!event.shiftKey&&key==="s"){event.preventDefault();if(state.editing)saveEditing();return;}if(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement||target instanceof HTMLSelectElement||target?.isContentEditable){if(event.key==="Escape")target.blur();return;}if(event.key==="Escape"){closeToolbarMenus();closeMediaPanel();closeSidebarOnNarrow();}else if(key==="-"){state.fontSize-=1;applyFontSize();}else if(["+","="].includes(key)){state.fontSize+=1;applyFontSize();}});

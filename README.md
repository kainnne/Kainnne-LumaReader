...
The web edition reuses the complete reader interface without requiring an account. It can open, drag in, or create up to three Markdown or plain-text documents, remove individual documents, and edit with the same preview and synchronized scrolling controls. A decorated **Share this Markdown** action creates an eight-character Cloudflare short link with a document-specific social preview. Share records expire after 30 days; the unchanged built-in sample reuses the permanent `/web/` address without consuming storage. Ordinary document content stays in browser memory, and no durable cloud library, file download, folder library, or PDF export is provided. When the browser grants a writable file handle, saving can update the original local file directly.

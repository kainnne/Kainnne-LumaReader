...
function argumentsOf(args){
 ...
 if(update&&markdown)throw Error('--update 不可搭配 --markdown，以免取代既有文件。');
 return {output:output||'lumareader.html',markdown,...(update?{update:true}:{})};
}
...
if(require.main===module)(async()=>{const args=argumentsOf(process.argv.slice(2));if(args.help){console.log(help);return;}const result=await(args.update?require('./update.cjs').update(args):install(args));console.log((args.update?(result.unchanged?'已是最新載入版本：':'已更新載入版本：'):'已加入原版 LumaReader：')+result.target);if(result.version)console.log('嵌入版本：'+result.version+'；請重新部署這份 HTML，再重新整理網頁。');if(result.backup)console.log('原始備份：'+result.backup);console.log('用你的網站伺服器開啟這份 HTML 即可使用。草稿儲存在瀏覽器；未連接後台。');})().catch(error=>{console.error('LumaReader：'+error.message);process.exitCode=1;});

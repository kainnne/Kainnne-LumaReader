...
async function mount() {
 editor=mountLumaReader($('reader'),{document:draft,collapseLabel:'縮回編輯器',onChange:value=>{draft=value;clearTimeout(backupTimer);backupTimer=setTimeout(()=>{try{persist(draft);}catch(e){error(e);}},250);},onSave:async value=>persist(value),onError:error,onExpand:expanded=>{$('expand-reader').textContent=expanded?'縮回編輯器':'放大編輯器';$('expand-reader').setAttribute('aria-pressed',String(expanded));}});
 await editor.ready;
}
$('expand-reader').onclick=()=>{if($('expand-reader').getAttribute('aria-pressed')==='true')editor.collapse();else editor.expand();};
$('backup').onclick=async()=>{try{const current=await editor.getDocument();const url=URL.createObjectURL(new Blob([current.markdown],{type:'text/markdown;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='transcript.md';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}catch(e){error(e);}};

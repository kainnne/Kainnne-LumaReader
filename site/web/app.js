(function () {
  ...
  function updateLibraryDisplay(){const label=window.lumaWeb?(webSessionLabels[state.language]||webSessionLabels.en):(state.libraryRoot?folderName(state.libraryRoot):t("noLibraryShort"));libraryRootEl.textContent=label;libraryRootEl.title=label;updateCreateAvailability();}
  ...
  async function chooseWebFiles(){
    ...
    try{
      ...
      if(result.path)await openProjectFile(result.path);

...
test("download redirects increment one platform atomically", async () => {
  ...
  assert.equal(response.headers.get("Location"), "https://github.com/kainnne/Kainnne-LumaReader/releases/download/v1.4.0/Kainnne-LumaReader-1.4.0-macOS-universal.dmg");
...
test("Linux downloads increment Linux only and use the published AppImage name", async () => {
  ...
  assert.equal(response.status, 302);
  assert.equal(response.headers.get("Location"), "https://github.com/kainnne/Kainnne-LumaReader/releases/download/v1.4.0/Kainnne-LumaReader-1.4.0-Linux-x64.AppImage");
...
test("download HEAD requests check all platform destinations without changing counters", async () => {
  ...
  for (const platform of ["macos", "windows", "linux"]) {
    ...
    assert.equal(response.status, 302);
    assert.match(response.headers.get("Location"), /releases\/download\/v1\.4\.0\//);

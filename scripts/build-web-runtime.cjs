...
// Git may check text files out as CRLF on Windows; compare equivalent text.
...
async function build({check=false}={}){
 ...
 if(check){if(canonical(await fs.readFile(editorOutput,'utf8'))!==canonical(editorCode))throw Error('Web direct editor is stale; run npm run build:web');}
 ...
 for(const [output,inputs] of Object.entries(groups)){
  ...
  if(check){if(canonical(await fs.readFile(path.join(root,output),'utf8'))!==canonical(content))throw Error(`${output} is stale; run npm run build:web`);}

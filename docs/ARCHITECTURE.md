...
Drag-and-drop feeds the same in-memory import path as the file picker and therefore preserves the three-document limit. Sharing serializes the current Markdown name and text, compresses it when the browser supports `CompressionStream`, and stores the result in the URL fragment after `#share=`. URL fragments are not sent to the static host. Opening the link reconstructs and sanitizes the Markdown locally; a size cap prevents oversized or decompression-heavy share payloads.
